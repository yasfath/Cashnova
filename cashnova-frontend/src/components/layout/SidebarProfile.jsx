...
const SidebarProfile = ({ setMobileOpen }) => {
  ...
  const updateActionCardPosition = useCallback((cardName = activeCard) => {
    ...
    const cardHeightEstimate =
      cardName === "editProfile" ? 300 : cardName === "password" ? 235 : 165;
    const left =
      window.innerWidth >= 1024
        ? Math.min(profileRect.right + gap, window.innerWidth - cardWidth - 24)
        : 16;
    const preferredTop =
      window.innerWidth >= 1024
        ? window.innerHeight - cardHeightEstimate - 88
        : profileRect.bottom - cardHeightEstimate - gap;
    const top = Math.min(
  ...
  const showCard = (cardName) => {
    updateActionCardPosition(cardName);
  ...
  const handleAddAccountSubmit = (event) => {
    ...
    if (!account.name || !account.email) {
      setMessage("Enter name and email to add the account.");
      return;
    }
  ...
  const handlePhotoSelected = (event) => {
    ...
    reader.onload = async () => {
      ...
      setPhotoMenuOpen(false);
      setSidebarPhotoOpen(false);
      setMessage("Photo selected. Click Upload to save it.");
    };
  ...
  const handleOpenCamera = async () => {
    ...
    if (!navigator.mediaDevices?.getUserMedia) {
      setMessage("Camera is not available here. Choose a camera photo.");
      openCameraFallback();
      return;
    }
    ...
    try {
      ...
      setMessage("Camera permission blocked or no camera found. Choose a camera photo.");
      setCameraOpen(false);
      openCameraFallback();
    }
  };
  ...
  const handleCapturePhoto = () => {
    ...
    setMessage("Photo captured. Click Done to upload it.");
  ...
  const handlePhotoUpload = async () => {
    if (!pendingAvatarUrl) {
      setMessage("Choose a photo from Camera or Files first.");
      openFilePicker();
      return;
    }
  ...
  const handleCameraDone = async () => {
    ...
    const capturedAvatarUrl = pendingAvatarUrl;
    setCameraOpen(false);
    await uploadProfilePhoto(capturedAvatarUrl);
  };
  ...
  return (
    <div ref={menuRef} className="relative mt-5">
      ...
      >
        ...
        >
          <div className="flex items-center">
            <span className="truncate text-[12px] font-semibold text-white">{profileName}</span>
          </div>
          <p className="truncate text-[11px] font-medium text-[#c6d7df]">
      ...
      {menuOpen && (
        <div className="profile-popover-panel theme-card absolute bottom-[calc(100%+10px)] left-0 z-30 w-full rounded-[22px] p-3">
          <div className="space-y-2">
            <button
              type="button"
              onClick={handleAddAccount}
              className="flex w-full items-center gap-3 rounded-2xl px-2 py-1 text-left text-[12px] font-semibold text-[#173847] transition hover:bg-white/55"
            >
      ...
      {sidebarPhotoOpen && (
        <div className="profile-popover-panel theme-card absolute bottom-[calc(100%+10px)] left-0 z-50 w-full min-w-[260px] rounded-[24px] p-3">
          <div className="space-y-2">
            <button
              type="button"
          ...
          >
            {pendingAvatarUrl ? (isSaving ? "Uploading..." : "Upload") : "Choose photo"}
          </button>
          {message ? <p className="mt-2 text-xs font-semibold text-[#1f5f7a]">{message}</p> : null}
        </div>
      )}

      {accountSwitcherOpen && (
        <div className="profile-popover-panel theme-card absolute bottom-[calc(100%+10px)] left-0 z-50 w-full min-w-[260px] rounded-[26px] p-3">
      ...
      {activeCard && createPortal(
        ...
        >
          <div
            className="profile-action-card pointer-events-auto theme-card fixed max-h-[calc(100vh-32px)] overflow-y-auto rounded-[18px] p-3 shadow-[0_22px_46px_rgba(9,30,43,0.16)]"
            ref={actionCardRef}
            style={{
              left: `${actionCardPosition?.left ?? 16}px`,
              top: `${actionCardPosition?.top ?? 16}px`,
              width: `${actionCardPosition?.width ?? 300}px`,
              maxHeight: actionCardPosition
                ? `calc(100vh - ${actionCardPosition.top + 16}px)`
                : "calc(100vh - 32px)",
            }}
          ...
          >
            <div className="mb-2">
              <div>
                <h2 className="text-[16px] font-extrabold text-[#12303f]">
                  {activeCard === "password"
                    ? "Change Password"
                ...
                <p className="mt-0.5 text-[11px] font-medium text-[#647c8a]">
                  {activeCard === "password"
            ...
            {activeCard === "password" ? (
              <form onSubmit={handlePasswordSubmit} className="space-y-2">
                <label className="block">
                  <span className="text-[12px] font-semibold text-[#355465]">Old Password</span>
                  <input
                    type="password"
                    value={passwordForm.oldPassword}
                    onChange={(event) => setPasswordForm((form) => ({ ...form, oldPassword: event.target.value }))}
                    className="theme-input mt-1 w-full rounded-2xl px-3 py-2 text-[12px] font-medium text-[#12303f] outline-none"
                    placeholder="Old Password"
                    required
                  />
                </label>
                <label className="block">
                  <span className="text-[12px] font-semibold text-[#355465]">New Password</span>
                  <input
                    type="password"
                ...
                <label className="block">
                  <span className="text-[12px] font-semibold text-[#355465]">Confirm Password</span>
                  <input
                    type="password"
                ...
                <button type="submit" className="theme-button-primary w-full rounded-2xl px-4 py-2 text-[12px] font-bold text-white" disabled={isSaving}>
              ...
              <form onSubmit={handleAddAccountSubmit} className="space-y-2">
                <label className="block">
                  <span className="text-[12px] font-semibold text-[#355465]">Name</span>
                  <input
                    type="text"
                ...
                <label className="block">
                  <span className="text-[12px] font-semibold text-[#355465]">Email</span>
                  <input
                    type="email"
                ...
                <button type="submit" className="theme-button-primary w-full rounded-2xl px-4 py-2 text-[12px] font-bold text-white">
                  Add Account
                </button>
              </form>
            ) : (
              <form onSubmit={handleProfileSubmit} className="space-y-3">
                <div className="flex flex-col items-center gap-2 border-b border-[rgba(98,125,141,0.16)] pb-3">
                  ...
                  {photoMenuOpen ? (
                    <div className="theme-card grid w-full grid-cols-2 gap-2 rounded-[18px] p-2">
                      <button
                        type="button"
                        onClick={handleOpenCamera}
                        className="flex items-center justify-center gap-2 rounded-2xl px-3 py-2.5 text-sm font-semibold text-[#173847] transition hover:bg-white/55"
                      >
                        <Camera size={16} />
                        <span>Camera</span>
                      </button>
                  ...
                  <div className="grid w-full grid-cols-2 gap-2">
                    <button
                      type="button"
                      onClick={handleOpenCamera}
                      className="theme-button-soft inline-flex items-center justify-center gap-2 rounded-2xl px-3 py-2 text-xs font-bold transition"
                    >
                      <Camera size={14} />
                      Camera
                    </button>
                  ...
                  <button
                    ...
                    onClick={handlePhotoUpload}
                    disabled={isSaving}
                    className="theme-button-primary w-full rounded-2xl px-5 py-2.5 text-sm font-bold text-white disabled:cursor-not-allowed disabled:opacity-55"
                  >
                    {pendingAvatarUrl ? (isSaving ? "Uploading..." : "Upload") : "Choose photo"}
                  </button>
                </div>
                ...
                <label className="block">
                  ...
                  <input
                    ...
                    className="theme-input mt-1.5 w-full rounded-2xl px-4 py-2.5 text-[12px] font-medium text-[#12303f] outline-none"
                    placeholder="Name"
                    required
                  />
                </label>
                ...
                <button type="submit" className="theme-button-primary w-full rounded-2xl px-5 py-2.5 text-[12px] font-bold text-white" disabled={isSaving}>
                  {isSaving ? "Saving..." : "Edit Profile"}
                </button>
              </form>
            )}

            {message ? <p className="mt-3 text-[12px] font-semibold text-[#1f5f7a]">{message}</p> : null}
          </div>
        </div>,
        document.body
      )}
            ...
            {pendingAvatarUrl ? (
              <div className="flex gap-3">
                ...
                <button
                  type="button"
                  onClick={handleCameraDone}
                  className="rounded-full bg-white px-6 py-3 text-sm font-extrabold text-[#12303f] shadow-[0_12px_28px_rgba(0,0,0,0.24)] transition hover:bg-[#f2fbfc]"
                >
                  Use photo
                </button>
              </div>
              ...
              <button
                ...
                className="flex items-center gap-3 rounded-full border-4 border-white/85 bg-white px-6 py-3 text-sm font-extrabold text-[#12303f] shadow-[0_18px_34px_rgba(0,0,0,0.28)] transition hover:bg-[#f2fbfc]"

...
export const AppDataProvider = ({ children }) => {
  ...
  const [sessionDataKey, setSessionDataKey] = useState(getSessionDataKey);
  ...
  const loadEntries = useCallback(async () => {
    ...
    if (backendConfigured) {
      try {
        ...
        if (normalizedEntries.length > 0 || localEntries.length === 0) {
          setEntries(normalizedEntries);
          writeLocalEntries(normalizedEntries);
          setDataSource("backend");
          setNotice("");
        } else {
          setEntries(localEntries);
          setDataSource("local");
          setNotice("Showing your locally saved entries for this account.");
        }
  ...
  useEffect(() => {
    const handleSessionChange = () => {
      setSessionDataKey(getSessionDataKey());
    };

    window.addEventListener("cashnova:auth-changed", handleSessionChange);
    window.addEventListener("storage", handleSessionChange);

    return () => {
      window.removeEventListener("cashnova:auth-changed", handleSessionChange);
  ...
  useEffect(() => {
    loadEntries();
  }, [loadEntries, sessionDataKey]);

  useEffect(() => {
    loadPreferences();
  }, [loadPreferences, sessionDataKey]);

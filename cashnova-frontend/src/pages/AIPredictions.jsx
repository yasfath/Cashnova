...
const RingCard = ({ title, value, sub1, sub2, progress = 0, tint = "purple" }) => {
  ...
  return (
    <div className="prediction-ring-card theme-card rounded-[18px] p-3 text-center transition duration-200 hover:-translate-y-[2px] hover:shadow-[0_16px_34px_rgba(19,52,72,0.08)] active:scale-[0.985]">
      <p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-[#6f8795]">{title}</p>

      <div
        className={`relative mx-auto mb-2.5 mt-2 flex h-[68px] w-[68px] items-center justify-center rounded-full bg-gradient-to-br ${palette.track} ${palette.glow}`}
        style={{
          backgroundImage: `conic-gradient(${palette.accent} ${safeProgress * 3.6}deg, rgba(224,235,240,0.86) 0deg)`,
        }}
      >
        <div className="absolute inset-[7px] rounded-full bg-[linear-gradient(180deg,rgba(255,255,255,0.96),rgba(244,249,250,0.9))] shadow-[inset_0_1px_0_rgba(255,255,255,0.85)]" />
        <div className="relative">
          <span className="text-[18px] font-extrabold tracking-[-0.02em] text-[#12303f]">
            {value}
          </span>
        </div>
      </div>

      <p className="text-[12px] font-bold text-[#173847]">{sub1}</p>
      <p className="mx-auto mt-0.5 max-w-[330px] text-[10px] leading-4 text-[#647c8a]">{sub2}</p>
    </div>
  );
};
...
const TopLineChart = ({ points, summaryLabel, currentBalance, forecastNet }) => {
  ...
  return (
    <div className="prediction-main-card theme-card rounded-[18px] p-3 transition duration-200 hover:-translate-y-[2px] hover:shadow-[0_18px_36px_rgba(19,52,72,0.08)] active:scale-[0.985]">
      <div className="mb-2 flex flex-wrap items-start justify-between gap-2">
        <div>
          <p className="text-[10px] font-semibold uppercase tracking-[0.22em] text-[#6f8795]">
            Forecast View
          </p>
          <h3 className="mt-1 text-[16px] font-bold tracking-[-0.02em] text-[#12303f]">
            Balance Forecast - Next 90 Days
          </h3>
          <p className="mt-1 max-w-[430px] text-[11px] leading-4 text-[#647c8a]">{summaryLabel}</p>
        </div>

        <div className="grid gap-2 sm:grid-cols-2">
          <div className="theme-panel rounded-[14px] px-3 py-2">
            <p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-[#6f8795]">
              Current balance
            </p>
            <p className="mt-1 text-[16px] font-bold text-[#12303f]">{currentBalance}</p>
          </div>
          <div className="theme-panel rounded-[14px] px-3 py-2">
            <p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-[#6f8795]">
              Forecast net
            </p>
            <p className="mt-1 text-[16px] font-bold text-[#1F5F7A]">{forecastNet}</p>
          </div>
        </div>
      </div>

      <div className="theme-panel rounded-[16px] p-2.5">
        <div className="mb-1.5 flex items-center gap-3 text-[10px] font-semibold text-[#647c8a]">
        ...
        <svg viewBox="0 0 520 220" className="h-[168px] w-full overflow-visible">
          <defs>
        ...
        <div className="mt-1 grid grid-cols-8 text-center text-[10px] font-semibold text-[#647c8a]">
          {points.map((point) => (
  ...
  return (
    <div className="prediction-main-card theme-card rounded-[18px] p-3 transition duration-200 hover:-translate-y-[2px] hover:shadow-[0_18px_36px_rgba(19,52,72,0.08)] active:scale-[0.985]">
      <div className="mb-2 flex flex-wrap items-center justify-between gap-2">
        ...
        <div className="theme-panel rounded-full px-4 py-2 text-[10px] font-semibold uppercase tracking-[0.18em] text-[#647c8a]">
          Max {formatCurrency(maxExpense)}
        </div>
      </div>

      <div className="theme-panel rounded-[16px] p-2.5">
        <div className="flex h-[201px] items-end justify-between gap-3">
          {bars.map((bar, index) => (
            <div key={bar.label} className="flex flex-1 flex-col items-center gap-3">
              <div className="flex h-[158px] w-full items-end rounded-[16px] bg-[linear-gradient(180deg,rgba(255,255,255,0.45),rgba(229,238,242,0.7))] p-2 shadow-[inset_0_1px_0_rgba(255,255,255,0.85)]">
                <div
...
const AIPredictions = () => {
  ...
  return (
    <PageContainer>
      <div className="ai-predictions-page grid h-[calc(100vh-104px)] grid-rows-[minmax(0,1fr)_auto] gap-3 overflow-hidden px-4 pb-3 pt-3 sm:px-5 xl:px-6">
